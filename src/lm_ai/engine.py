...
class TransferDirector:
    def __init__(self, clubs: Iterable[Club], players: Iterable[Player], config: dict, seed: int = 26):
        self.clubs = {c.id: c for c in clubs}
        self.players = {p.id: p for p in players}
        self.config = config
        self.seed = seed

    def squad(self, club: Club) -> list[Player]:
        ids = set(club.player_ids)
        return [p for p in self.players.values() if p.id in ids or p.club_id == club.id]

    def calculate_needs(self, club: Club) -> dict[str, float]:
        targets = self.config.get("roster_targets", DEFAULT_TARGETS)
        starters = self.config.get("starter_slots", DEFAULT_STARTERS)
